...
userRoute.post("/check", authToken, async(req, res, next) => {
    try{
            ...
            .then(pool => {
                ...
                if (result.recordset.length > 0) {
                    const user = {
                        ...
                        mobile_no: result.recordset[0].mobile,
                    
                    };
                    // sendWhatsapp(result.recordset[0].mobile, result.recordset[0].otp);
                     sendEmail(result.recordset[0].email, "OTP Verification", result.recordset[0].otp);
...
function sendEmail(mailto, subject, otp,user,pass) {
...
const sendWhatsapp = async (mobile, otp) => {
    ...
    await axios.get("https://md1.enotify.app/api/sendText?token=" + token + "&phone=91" + mobile + "&message=" + "cgh" + "")
        .then(response => {
